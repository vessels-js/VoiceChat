...
public class GuiScreenOptionsUI extends GuiScreen {
	...
	public void drawScreen(int x, int y, float time) {
		...
		glPopMatrix();
		glPushMatrix();
		glTranslatef(width / 2 - (fontRenderer.getStringWidth(StringTranslate.getInstance().translateKey("menu.uiOptions")) / 2), 12, 0);
		drawString(mc.fontRenderer, StringTranslate.getInstance().translateKey("menu.uiOptions"), 0, 12, -1);
		glPopMatrix();
	...
	public void initGui() {
		...
		buttonList.add(new GuiButton(1, (this.width / 2) - 75, 73, 150, 20, StringTranslate.getInstance().translateKey("menu.resetAll")));
		buttonList.add(new GuiButton(2, (this.width / 2) - 150, 50, 150, 20, StringTranslate.getInstance().translateKey("menu.uiPlacement")));
		buttonList.add(opacity = new GuiBoostSlider(-1, this.width / 2 + 2, 50, "", StringTranslate.getInstance().translateKey("menu.uiOpacity") + ": " + (voiceChat.getSettings().getUIOpacity() == 0 ? StringTranslate.getInstance().translateKey("options.off") : (int) (voiceChat.getSettings().getUIOpacity() * 100) + "%"), 0));
	...
	public void updateScreen() {
		...
		opacity.setDisplayString(StringTranslate.getInstance().translateKey("menu.uiOpacity") + ": " + (voiceChat.getSettings().getUIOpacity() == 0 ? StringTranslate.getInstance().translateKey("options.off") : (int) (voiceChat.getSettings().getUIOpacity() * 100) + "%"));
	}
}
