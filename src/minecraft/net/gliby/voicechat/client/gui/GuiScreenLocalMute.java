...
public class GuiScreenLocalMute extends GuiScreen {
	...
	class List extends GuiSlot {
		...
		@Override
		protected void drawSlot(int valueId, int par1, int par2, int par3, Tessellator tessellator) {
			GuiScreenLocalMute.this.drawCenteredString(GuiScreenLocalMute.this.fontRenderer, VoiceChatClient.getSoundManager().playerMutedData.get(VoiceChatClient.getSoundManager().playersMuted.get(valueId)), GuiScreenLocalMute.this.width / 2, par2 + 1, 16777215);
			GuiScreenLocalMute.this.drawCenteredString(GuiScreenLocalMute.this.fontRenderer, "\247lX", GuiScreenLocalMute.this.width / 2 + 88, par2 + 3, 0xff0000);
		}
		 ...
		 */
		@Override
		protected void elementClicked(int index, boolean p_148144_2) {
	...
	private void actionPerformed(int button) {
		...
		case 0:
			...
			if (entityPlayer != null) {
				if (entityPlayer.worldObj.isRemote && !VoiceChatClient.getSoundManager().playersMuted.contains(entityPlayer.entityId)) {
					VoiceChatClient.getSoundManager().playersMuted.add(entityPlayer.entityId);
					VoiceChatClient.getSoundManager();
					ClientStreamManager.playerMutedData.put(entityPlayer.entityId, entityPlayer.getCommandSenderName());
				}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		this.drawCenteredString(this.fontRenderer, StringTranslate.getInstance().translateKey("menu.mutedPlayers"), this.width / 2, 16, -1);
		if (playerNotFound) {
			this.drawCenteredString(this.fontRenderer, "\247c" + StringTranslate.getInstance().translateKey("commands.generic.player.notFound"), this.width / 2, height - 59, -1);
		}
	...
	public void initGui() {
		...
		playerTextField = new GuiTextField(this.fontRenderer, width / 2 - 100, this.height - 57 - heightOffset, 130, 20);
		playerTextField.setFocused(true);
		this.buttonList.add(this.doneButton = new GuiButton(0, this.width / 2 + 32, this.height - 57 - heightOffset, 98, 20, StringTranslate.getInstance().translateKey("menu.add")));
		this.buttonList.add(this.doneButton = new GuiButton(1, this.width / 2 - 75, this.height - 32 - heightOffset, StringTranslate.getInstance().translateKey("gui.done")));
		this.listPlayers = new GuiScreenLocalMute.List();
		this.listPlayers.registerScrollButtons(null, 7, 8);
	}
