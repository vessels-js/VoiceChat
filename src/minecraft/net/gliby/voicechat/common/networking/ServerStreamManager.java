...
public class ServerStreamManager {
				...
				if (target.entityId != speaker.entityId) {	
					...
					if (distanceBetween < distance * distance) {
						...
						if (stream.tick % voiceChat.serverSettings.positionUpdateRate == 0) {
							if (distanceBetween > 64 * 64) voiceChat.getVoiceServer().sendEntityPosition(target, speaker.entityId, speaker.posX, speaker.posY, speaker.posZ);
							stream.tick = 0;
						}
						stream.tick++;
					}
	...
	public void giveEntity(EntityPlayerMP receiver, EntityPlayerMP speaker) {
		voiceChat.getServerNetwork().sendEntityData(receiver, speaker.entityId, speaker.getCommandSenderName(), speaker.posX, speaker.posY, speaker.posZ);
