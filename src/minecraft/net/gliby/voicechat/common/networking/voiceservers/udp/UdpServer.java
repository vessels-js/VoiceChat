	...
	protected synchronized void fireUdpServerPacketReceived() {
		...
		for (final UdpServer.Listener l : ll) {
			try {
				...
				LOGGER.log(Level.WARNING, "UdpServer.Listener " + l + " threw an exception: " + exc.getMessage());
				fireExceptionNotification(exc);
			} // end catch
	...
	protected void runServer() {
		try {
			...
			try {
				...
				LOGGER.log(Level.WARNING, String.format("Could not set receive buffer to %d. It is now at %d. Error: %s", pl, bl, exc.getMessage()));
			} // end catch
			...
			if (gg != null) {
				...
				for (final String p : proposed) { // See which ones are valid
					try {
						...
						LOGGER.log(Level.WARNING, "Could not join " + p + " as a multicast group: " + exc.getMessage());
					} // end catch
			...
			synchronized (This) {
				if (this.currentState == State.STOPPING) { // User asked to stop
					...
					LOGGER.log(Level.WARNING, "If the server cannot bind: Switch to Minecraft Networking in config or setup UDP properly, that means port-forwarding.");
